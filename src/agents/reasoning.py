...
class ReasoningAgent(BaseAgent):
    ...
    async def execute_stream(self, context: AgentContext):
        ...
        Yields (event_type, data_dict) tuples during the ReAct loop.
        The final yield has event_type="_result" with an AgentResult.
        """
        start = time.perf_counter()
        tools_used = []
        ...
        for iteration in range(self._max_iterations):
            yield ("status", {
                "step": "reasoning",
                "message": f"ReAct iteration {iteration + 1}/{self._max_iterations}",
            })
            ...
            for tool_call in choice.message.tool_calls:
                ...
                yield ("status", {
                    "step": "tool_call",
                    "message": f"Calling {func_name}({json.dumps(func_args)})",
                })
                ...
                yield ("status", {
                    "step": "tool_result",
                    "message": f"{func_name} → {str(tool_output)[:200]}",
                })
